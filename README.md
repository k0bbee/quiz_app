...
题型、难度和知识点权重都按“相对权重”处理，而不是要求用户手动凑满 100%。例如两个知识点分别填 `100` 和 `80` 时，实际有效占比会归一化为约 `56%` 和 `44%`。AI 出题窗口和设置页都提供“更新权重显示”按钮，只有用户确认后才刷新有效占比预览，避免滚动或误触输入框时连带改变显示结果。
...
AI 出题会按小批次生成候选题，再在本地按题型、难度和知识点权重筛选。若模型返回的 JSON 疑似因为输出过长被截断（例如 `Unterminated string`），程序会自动降低候选批次重试；如果降到小批次仍失败，会显示 `GEN-AI-JSON-001`，此时应减少单次题量、缩小知识点/题型覆盖范围，或换用支持更大输出上限的模型/端点。
