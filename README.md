...
API Key 读取优先级：环境变量 `QUIZ_APP_API_KEY` → 系统密钥环。设置页不会回显已有密钥；空输入表示保持不变，只有输入新值才会更新，清除操作需要单独确认。

新密钥会立即进入当前应用会话，并在系统密钥环可用时持久化。密钥环不可用或写入失败时只保留当前会话，不再自动写入 `data/settings.json`。程序仍可读取旧版本留下的明文字段用于迁移，但下一次显式更新或清除密钥会移除该字段。
