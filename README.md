...
| 📝 **题目集练习** | 按多个知识点/难度筛选题目集，支持题集重命名、随机出题 |
...
- 应用数据导入导出：设置页支持 `.quizdata` 数据包，用于迁移课程、题库、题目集、进度、已掌握主题、当前课程指针和非敏感设置。
...
- 可标记本题**不确定**，结果页会单独显示“答对但不确定”数量
- 结果页会给出下一步建议，并可直接重做错题、不确定题或整套题，减少复习路径跳转
- 配对题需完成所有匹配才可提交；排序题若未调整顺序，提交前会确认
- 快捷键：`1-9` 选选项，`Enter` 提交/下一题，`Esc` 退出
- 退出时**自动保存草稿**，首页可继续未答题目，不需要重做已答部分
