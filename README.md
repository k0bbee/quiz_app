...
3. **开始练习** — 从题目集中选题，支持多知识点筛选、单选/判断/配对/排序/填空/简答/情景题
...
| 功能 | 说明 |
|---|---|
| 📚 **课件管理** | 导入任意课程文件夹，自动提取主题和摘要，支持课程重命名 |
| 📝 **题目集练习** | 按多个知识点/难度筛选题目集，支持题集重命名、随机出题和完整草稿恢复 |
| 🤖 **AI 出题** | 调用 LLM API 基于课件内容生成双语题目，创建时可自定义题集名称 |
...
| 📊 **进度面板** | 总览练习统计、按主题正确率与掌握度、近期记录，支持标记/取消“已掌握”并给出下一步复习建议 |
| 🗂 **题库管理** | 按题集/难度/关键词查看散题，紧凑预览、编辑/新建单道题目，支持多选和批量删除 |
| ⚙ **设置** | 中英文切换、API Key/模型/端点配置、练习默认题量/难度/计时器/出题模板/题型与难度权重、环境检查、进度与应用数据导入导出 |
| 🌐 **双语切换** | 全界面实时中英切换，题目同时显示双语版本 |

## 权重配置说明

题型、难度和知识点权重都按“相对权重”处理，而不是要求用户手动凑满 100%。例如两个知识点分别填 `100` 和 `80` 时，实际有效占比会归一化为约 `56%` 和 `44%`。AI 出题窗口的知识点权重只显示当前已选主题；标签格式为 `原始权重 (有效占比)`，避免和配置变更的 `旧值 → 新值` 混淆。AI 出题窗口和设置页都提供“更新权重显示”按钮，只有用户确认后才刷新有效占比预览，避免滚动或误触输入框时连带改变显示结果。
...
- 题库管理：题目列表可按题集查看，使用单行紧凑预览，完整题干可悬停查看；单选时编辑题目，多选时禁用编辑并允许批量删除，删除会同步清理题目集引用，题集被删空后会自动移除。
...
- 结果页会给出下一步建议，并可直接重做错题、不确定题、复查题或整套题，减少复习路径跳转
...
AI 出题会按小批次生成候选题，再在本地按题型、难度和知识点权重筛选。小题量生成不会额外超采样；较大题量才会请求少量候选题来抵消模型漂移。生成状态会显示当前批次、已接受题数、耗时和可取消提示。若模型返回的 JSON 疑似因为输出过长被截断（例如 `Unterminated string`），程序会自动降低候选批次重试；如果降到小批次仍失败，会显示 `GEN-AI-JSON-001`，此时应减少单次题量、缩小知识点/题型覆盖范围，或换用支持更大输出上限的模型/端点。
