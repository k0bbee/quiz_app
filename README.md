...
| 📚 **课件管理** | 导入任意课程文件夹，自动提取主题和摘要，显示文件/阶段进度并可安全停止，支持课程重命名和按稳定知识点配置考试范围 |
...
- 课程与题集管理：导入课程时可填写课程名；AI 出题创建题集时可填写题集名；已导入课程和已生成题集都可以重命名。删除课程前会显示关联题目、题集、学习记录和草稿数量，并允许保留练习数据、解除课程关联，或清理关联题库；历史学习记录不会被静默删除。
