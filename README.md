...
- 课程与题集管理：导入课程时可填写课程名；AI 出题创建题集时可填写题集名；已导入课程和已生成题集都可以重命名。删除课程时可选择原样保留题库、解除课程关联，或清理关联题库；未完成草稿会取消，历史真题会保留并改为未归属。新完成的练习会直接保存课程、题集和逐题不可变复盘快照；可在进度页的最近记录中双击或按回车打开。即使原题或题集已不可用，已成功归档的题干、作答、正确答案、解析和来源仍可复盘，但相关重练动作会禁用并说明原因。
