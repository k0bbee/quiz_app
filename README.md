...
| 🎯 **错题复习** | 根据历史答题记录计算复习优先级，优先安排反复错、最近错和低掌握度题目 |
| 📊 **进度面板** | 总览练习统计、按主题正确率、近期记录，并给出下一步复习建议 |
...
- 进度导入导出：设置页支持进度 JSON 导入导出。
- 应用数据导入导出：设置页支持 `.quizdata` 数据包，用于迁移课程、题库、题目集、进度、当前课程指针和非敏感设置。
