...
- 自定义 AI 端点仅在本地显式配置后，才会收到对应凭据和所选课程上下文；请求不跟随重定向，响应正文有固定上限。
- 本地 Agent 执行受到能力限制：禁用工具、会话持久化和项目加载，并运行在隔离工作目录中。不支持无工具隔离的 CLI 会被拒绝，输出由有界管道持续读取。
- 课程文件夹中的符号链接和解析后逃出所选目录的路径不会读取；文件数、总源数据、Office 成员/页面和抽取文本均有资源预算。
- 超大、成员过多或压缩比异常的 `.quizdata` 数据包会在写入应用数据前被拒绝。
