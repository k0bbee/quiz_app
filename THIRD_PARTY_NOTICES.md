...
`requirements.txt`目前声明最低兼容版本而非锁定版本。下表“审计环境版本”来自2026年7月21日实际测试环境，仅用于当前审计；制作V1.0.0发布包和软件著作权材料时，必须从冻结环境重新生成版本与许可证清单。
...
| PyMuPDF | 1.27.2.3 | PDF文本提取和扫描页渲染 | AGPL-3.0或Artifex商业许可；没有商业许可证证明时不得按商业许可处理 | [PyMuPDF许可说明](https://pymupdf.readthedocs.io/en/latest/about.html#license) |
| Pillow | 12.1.1 | OCR图像转换 | MIT-CMU | [Pillow LICENSE](https://github.com/python-pillow/Pillow/blob/main/LICENSE) |
...
当前 `requirements.txt` 中的全部直接依赖均已在上表列出：PyQt6、requests、keyring、PyMuPDF、Pillow和pytesseract。PyQt6-Qt6、PyQt6-sip和Tesseract因其对实际运行或分发边界重要而一并列示。
...
- 当前根许可证为GPL-3.0-only，与实际分发包的全部义务不是同一概念。特别是PyMuPDF的AGPL-3.0/商业双重许可，需要按实际取得的许可和分发方式单独复核。
