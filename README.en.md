...
Course merging retains the selected target identity. Selecting source courses previews the affected questions, sets, complete archives, incomplete archives, legacy history awaiting migration, drafts, historical exams, and current-event packs. Before a course is merged or deleted, legacy completed records are backfilled from any questions, sets, and course metadata that still exist. Missing fields remain explicitly marked, and the lifecycle action stops if the archive cannot be saved. Live assets move to the retained course, while completed records preserve the course identity captured when the session occurred. Source courses are removed only after every linked-data write succeeds; failures or cancellation trigger rollback, and migrated past exams must be analyzed again before their profiles are reused.
