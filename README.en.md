...
Course merging retains the selected target identity. Selecting source courses previews the affected questions, sets, completed history, drafts, historical exams, and current-event packs. Live assets move to the retained course, while completed records preserve the course identity captured when the session occurred. Source courses are removed only after every linked-data write succeeds; failures or cancellation trigger rollback, and migrated past exams must be analyzed again before their profiles are reused.

Deleting a course reports its linked questions, sets, completed history, unfinished drafts, past exams, and current-event packs before confirmation. Unfinished drafts are cancelled, while immutable completed-session review snapshots are always preserved. Past-exam records and their source copies remain but become unassigned and require analysis again; course-specific current-event packs are deleted. A failed course deletion restores the affected data. Newly completed sessions preserve course, set, and per-question review snapshots; double-click a recent session on the Progress page, or select it and press Enter, to reopen it. Stems, submitted and correct answers, explanations, and source references remain reviewable when the original questions or set no longer exist, while unavailable retry actions are disabled with an explanation.
