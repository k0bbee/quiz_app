...
class GenerationDialogUiTests(unittest.TestCase):
    ...
    def test_dialog_shows_generation_plan_preview_from_current_controls(self):
            ...
            self.assertIn("选择题: 5", preview)
            self.assertIn("情境选择题: 3", preview)
            self.assertIn("判断题: 2", preview)
            self.assertIn("难度分布", preview)
            self.assertIn("简单: 2", preview)
            self.assertIn("中等: 5", preview)
            self.assertIn("困难: 3", preview)
            self.assertIn("组合计划", preview)
            self.assertIn("cache", preview)
            self.assertIn("选择题", preview)
            self.assertIn("定义回忆", preview)
            self.assertNotIn("multiple_choice", preview)
            self.assertNotIn("definition", preview)
